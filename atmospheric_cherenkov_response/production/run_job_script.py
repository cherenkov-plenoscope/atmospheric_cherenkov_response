...
tmp_dir = job["tmp"]["dir"]
pkey = job["particle_key"]
skey = job["site_key"]

os.makedirs(tmp_dir, exist_ok=True)

prng = np.random.Generator(np.random.PCG64(job["production"]["run_id"]))

config = json_utils.tree.read(op.join(work_dir, "config"))

site = config["sites"][skey]
particle = config["particles"][pkey]

site_particle_deflection = magnetic_deflection.read_deflection(
    op.join(work_dir, "magnetic_deflection", job["pointing_key"])
)[skey][pkey]
...
corsika_primary_steering = acr.particles.draw_corsika_primary_steering(
    ...
    num_events=job["production"]["num_showers"],
    prng=prng,
)
